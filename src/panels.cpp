...
static bool &panel_manager_initialized_state()
{
...
panel_manager::panel_manager()
{
    ...
    panel_manager_initialized_state() = true;
}

panel_manager::~panel_manager()
{
    panel_manager_initialized_state() = false;
}

bool panel_manager::is_initialized()
{
    return panel_manager_initialized_state();
}
...
void panel_manager::deserialize( const JsonArray &ja )
{
    ...
    for( JsonObject joLayout : joLayouts.get_array( "layouts" ) ) {
        ...
        for( JsonObject joPanel : joLayout.get_array( "panels" ) ) {
            ...
            saved_entries.emplace_back( id, toggle );
        ...
        saved_layout_entries[layout_id] =
            std::move( saved_entries );
    }
    if( ja.size() > 1 ) {
