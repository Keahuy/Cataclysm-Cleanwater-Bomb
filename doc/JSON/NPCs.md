...
| `<money:VAR>`                                 | N/A            | gets replaced with the context variable VAR formatted as money
...
`quote_npc_trade_item: `string or [variable object](#variable-object), (*optional* `count: `int or [variable object](#variable-object)), (*optional* `prefix: `string) | Quotes the named item using the current dialogue alpha talker as buyer and beta talker as seller, matching the NPC trade screen price calculation.  For items counted by charges, `count` is applied as charges before pricing.  Writes context variables `<prefix>_item_id`, `<prefix>_item_name`, `<prefix>_count`, and `<prefix>_cost`; `prefix` defaults to `quote`.
...
| item_rot()    |  ✅   |   ✅  | u, n  | Returns the rotting progress of a perishable item (typically 'food' or 'comestible'). Can only be used on an item.<br/>In any case, non-perishable items will return -1.<br/><br/>Optional kwargs:<br/>`format`: `relative`/`raw` - The format of the returned value. If set to `relative`, it ranges from 0 to 1 (from fresh to completely rotted). If set to `raw`, it returns the accumulated rotting time progress. Defaults to `relative`.<br/>`unit`: `t`/`s`/`m`/`h`/`d` - All recognizable time units. This is only effective when `format` is set to `raw`. If not specified, it defaults to turns as the unit.<br/><br/>Examples:<br/>`condition": { "math": [ "n_item_rot() >= 0" ] }`<br/>`{ "math": [ "n_item_rot() = 0.5" ] }`<br/>`{ "math": [ " n_item_rot('format':'raw','unit':'minute') -= min( n_item_rot('format':'raw','unit':'minute'), 10 )" ] }`|
| item_burnt(`s`/`v`)    |  ✅   |   ✅  | u, n  | Returns the internal burning progress of the item.<br/>Returns -1 if it is a zero-volume item (usually a debug item).<br/><br/>Optional kwargs:<br/>`format`: `raw`/`percent` - The format of the returned value. If set to `raw`, returns the burning progress as an integer. If set to `percent`, returns the percentage in a range of 0~100. Defaults to `raw`. Since `item.burnt` is in int format, assigning values with the `raw` parameter will truncate the decimal part.<br/><br/>Example:<br/>`"condition": { "math": [ "n_item_burnt() >= 0"] }`<br/>`{ "math": [ "n_item_burnt('format':'percent') = 0"] }`|
...
| sensitive()     |  ✅  |   ✅   | u, n  | Return or set the character's current sensitivity to external stimuli. Values below 100 dull and values above sharpen reactions such as pain gained, focus recovery and wetness morale. The value drifts towards its equilibrium over time. When assigning via math, fractional parts are truncated toward zero.<br/><br/>Example:<br/>`{ "math": [ "u_sensitive() -= 10" ] }` Lowers the alpha talker's sensitivity by 10<br/>|
| sensitive_mod()     |  ✅  |   ✅   | u, n  | Return or set the equilibrium that current sensitivity drifts towards. Clamped to 0..500. When assigning via math, fractional parts are truncated toward zero.<br/><br/>Example:<br/>`{ "math": [ "u_sensitive_mod() += 5" ] }` Raises the alpha talker's sensitivity equilibrium by 5<br/>|
