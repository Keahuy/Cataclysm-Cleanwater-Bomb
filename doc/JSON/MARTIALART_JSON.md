...
> [!IMPORTANT] **Moved / 已迁移**
>
> Stable document ID / 稳定文档 ID: `json.martial-arts`
> 中文: https://crimsoncrossbunker.github.io/CCB-Docs/reference/json/martial-arts/
> English: https://crimsoncrossbunker.github.io/CCB-Docs/en/reference/json/martial-arts/
...
```jsonc
{
  ...
  "aoe": "spin",                         // This technique has an area-of-effect. Doesn't proc against solo targets. Options are `aoe` (attacks adjacent nearby enemies), `impale` (hits target and one enemy behind it), and `wide` (hits enemies in an arc near the target)
