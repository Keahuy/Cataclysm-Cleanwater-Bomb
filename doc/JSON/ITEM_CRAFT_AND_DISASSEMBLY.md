...
"rot_inherit": "default",   // Optional (default: "default"). How the rot of crafting components carries over to the result. "default" picks automatically: results with a shorter shelf life than every component inherit the shortest remaining lifespan among components; cooked results (heated result or non-raw result) inherit the mass-weighted average component rot, floored at 5x the most rotten component's rot scaled by the going-bad components' mass fraction (the floor only activates when a near fully rotten component is present); everything else inherits the highest relative rot among components. Explicit values: "max" (always the highest relative rot), "weighted" (always the mass-weighted average), "shortest" (always the shortest remaining lifespan), "preserve_blend" (always the weighted average with the 5x bad-mass-fraction floor), "fresh" (the in-progress craft does not rot and the result stays fresh).
