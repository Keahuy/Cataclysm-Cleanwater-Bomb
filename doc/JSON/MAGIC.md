...
> [!IMPORTANT] **Moved / 已迁移**
>
> Stable document ID / 稳定文档 ID: `json.magic-spells-enchantments`
> 中文: https://crimsoncrossbunker.github.io/CCB-Docs/reference/json/magic-spells-enchantments/
> English: https://crimsoncrossbunker.github.io/CCB-Docs/en/reference/json/magic-spells-enchantments/
...
`max_hp_modifier`           | Modifies the maximum HP of the given `part` (a body_part_id, or `"all"` for every part).  Supports `add` and `multiply`.  Math expressions are accepted when the enchantment is defined as content (item/mutation `passive_effects`), but only plain numbers when the enchantment cache is loaded from a save or a runtime cache.  Characters only; evaluated on enchantment cache recalculation.
`limb_score_modifier`       | Modifies the limb score `score` (see `limb_scores.json`) provided by body parts.  With an optional `part` field the modifier only affects that body part's contribution, otherwise it applies to the aggregated score.  Supports `add` and `multiply`; math expressions are accepted when the enchantment is defined as content (item/mutation `passive_effects`), but only plain numbers when the cache is loaded from a save or a runtime cache.  A per-`part` modifier has no effect on body parts that do not provide the score at all (base contribution is 0).  For the `manip` score, per-`part` modifiers apply after the body part's natural limit (`limb_score_max`), so they can raise that part's contribution above its limit.  Characters only; evaluated on enchantment cache recalculation.
...
`BLEEDING_RATE`         | Affects the rate you lose blood when bleeding.
